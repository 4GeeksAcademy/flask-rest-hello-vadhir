...
def sign_up():
    ...
    if None in [name, email, password]:
        return jsonify ({
            "message": "Todos los datos deben ser llenados"
        }), 400
    user_already_exist = db.session.execute(db.select(User).filter_by(email=email)).one_or_none()
    if user_already_exist:
        return jsonify({
            "message": "No se puede crear el usuario, intenta más tarde"
        }), 400
...
def create_planet():
    ...
    if planet_already_exist:
        return jsonify({
            "message": "Ups, al parecer algún otro ser pensó igual que tú al establecer su propio planeta. Intenta con otro nombre."
        }), 400
